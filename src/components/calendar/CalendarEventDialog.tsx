    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent overlayClassName="bg-[#241a30]/45 backdrop-blur-sm" className="max-h-[calc(100dvh-2rem)] w-[calc(100%-1.5rem)] max-w-2xl overflow-y-auto overscroll-contain rounded-[28px] border-[#ddcfed] bg-[#fffaff] p-5 font-body shadow-2xl [scrollbar-color:#cdb8e4_transparent] [scrollbar-width:thin] sm:p-7 [&_h2]:font-body [&_h3]:font-body [&_h4]:font-body [&>button]:right-5 [&>button]:top-5 [&>button]:flex [&>button]:h-11 [&>button]:w-11 [&>button]:items-center [&>button]:justify-center [&>button]:rounded-full [&>button]:text-[#6b4c9a] [&>button]:focus:ring-[#7c3aed]">
        ...
        <form onSubmit={submit} className="space-y-6 sm:space-y-7">
          ...
          <textarea aria-label="Qué pasa después" value={form.afterNote || ''} onChange={event => onFormChange({ ...form, afterNote: event.target.value })} placeholder="¿Qué pasa después? (opcional) Ej: volvemos a casa y descansamos" className="h-16 w-full resize-none rounded-2xl border border-[#ede4f8] bg-[#faf8ff] p-3 text-base text-[#4a4a5a] outline-none focus:ring-2 focus:ring-[#6b4c9a]/20" />
          <textarea aria-label="Plan B" value={form.planB || ''} onChange={event => onFormChange({ ...form, planB: event.target.value })} placeholder="Plan B (opcional)" className="h-16 w-full resize-none rounded-2xl border border-[#ede4f8] bg-[#faf8ff] p-3 text-base text-[#4a4a5a] outline-none focus:ring-2 focus:ring-[#6b4c9a]/20" />
          <textarea aria-label="Preparación sensorial" value={form.sensoryNote || ''} onChange={event => onFormChange({ ...form, sensoryNote: event.target.value })} placeholder="Preparación sensorial (opcional)" className="h-16 w-full resize-none rounded-2xl border border-[#ede4f8] bg-[#faf8ff] p-3 text-base text-[#4a4a5a] outline-none focus:ring-2 focus:ring-[#6b4c9a]/20" />
