...
import {
  Activity, ArrowRight, CalendarDays, Check, CheckCircle2, Clock3, Heart,
  MessageCircle, Network, NotebookPen, Sparkles, Target, Users,
} from 'lucide-react';
import type { AcompanamientoData, CalendarEvent, EmotionalRecord, ProfessionalSession, User } from '@/data/api';
...
export interface PertenecienteDetailProps {
  ...
  activities?: Array<{ id: string; title: string; status: string; objective?: string; completedAt?: string | null }>;
...
export default function PertenecienteDetail({
  person, activities = [], emotions = [], events = [], sessions = [], supportData, role,
  canViewHistory = true, canManageSessions = false, onOpenChat, onOpenCalendar,
  onScheduleSession, onOpenPrivateNote, onCreateSharedNote, onDeleteSharedNote,
...
function SupportPill({ label, tone }: { label: string; tone: 'blue' | 'violet' }) { return <div className={`flex items-center gap-3 rounded-2xl p-3 ${tone === 'blue' ? 'bg-sky-50 text-sky-800' : 'bg-violet-50 text-violet-800'}`}><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/80"><Users size={17} /></span><span className="text-sm font-bold">{label}</span></div>; }
