...
describe("WheelPrecision interaction", () => {
  ...
  const setup = (correct = 0, segments = 4, useImages = false) => {
    ...
    const options = Array.from({ length: segments }, (_, index) =>
      useImages ? `https://example.com/${index}.png` : `opción ${index + 1}`,
    );
    const view = render(
      <MiniGame
        gameType="wheel"
        gameData={{
          wheel: {
            ...
            rounds: [
              { targetWord: "PERA", image: options[correct], options, correct },
            ],
          },
        }}
        onFinish={vi.fn()}
  ...
  it("accepts only one stop command and exposes the stopping state immediately", () => {
    ...
    expect(screen.getByRole("button", { name: /frenando/i })).toHaveAttribute(
      "aria-disabled",
      "true",
    );
  ...
  it("uses one stable button and ignores the synthetic click after pointerdown", () => {
    ...
    expect(
      screen.getByLabelText(/ruleta de 4/i).getAttribute("style"),
    ).toContain("rotate(");
  ...
  it.each([4, 6, 8])(
    "keeps all pictograms inside the wheel for %i segments",
    (segments) => {
      const { unmount } = setup(0, segments, true);
      const images = screen
        .getByLabelText(new RegExp(`ruleta de ${segments}`, "i"))
