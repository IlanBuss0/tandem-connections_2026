import { Image, MessageCircle, Stethoscope, Trophy, UserRound } from 'lucide-react';
...
const cards = [
  ...
  { title: 'Mis pictogramas', action: 'Ver pictogramas', tab: 'pictograms', icon: Image },
  ...
  return (
    <section className="mt-6 lg:hidden" aria-labelledby="secondary-access-title">
      ...
      <div className="flex snap-x gap-3 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <article className="min-w-[74%] snap-start rounded-3xl border border-[#e9def5] bg-white p-4 sm:min-w-[45%]">
          <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-[#f1e9fa]">{avatar && /^(https?:|data:image\/|\/|\.\/|\.\.\/)/.test(avatar) ? <img src={avatar} alt="" className="h-full w-full object-cover" /> : avatar || <UserRound size={20} aria-hidden />}</div><h3 className="mt-2 font-bold text-[#3f2c55]">Mi perfil</h3><p className="text-sm text-[#756a82]">Tu espacio personal</p><button onClick={() => onNavigate?.('profile')} className="mt-3 min-h-11 text-sm font-bold text-[#6b3faf] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7c3aed]">Ver mi perfil</button>
        </article>
