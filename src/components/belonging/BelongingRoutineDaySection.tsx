...
const reminderChoices = [
  ...
  { value: 5, label: "5 min después" },
  { value: 10, label: "10 min después" },
...
export default function BelongingRoutineDaySection({
  ...
  const deepRoutineId =
    initialRoutineId && routines.some((r) => r.id === initialRoutineId)
      ? initialRoutineId
      : null;
  ...
  const [manualPictogram, setManualPictogram] = useState<Pictogram | null>(
    null,
  );
  ...
  if (!canUseMiDia) {
    return (
      <section
        ...
        className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-amber-950"
      >
        <h3
          id="calendar-routine-title"
          className="text-sm font-extrabold uppercase tracking-wide text-[#5f477c]"
        >
          Rutina
        </h3>
        <p className="mt-1.5 text-xs leading-relaxed">
          Tu tutor deshabilitó temporalmente Mi día. No podés ver ni completar
          rutinas hasta que lo vuelva a habilitar.
        </p>
      </section>
    );
  }
  ...
  const saveDayEdit = () => {
    ...
    renameRoutine(
      active.id,
      dayName.trim() || "Mi día",
      dayDow === "null" ? null : (Number(dayDow) as DayKey),
      dayDate || undefined,
    );
    setEditingDay(false);
  };
  ...
  const openEdit = (it: RoutineItem) => {
    ...
    setForm({
      ...
      pictogramLabel: it.pictogramLabel || "",
  ...
  const submitItem = () => {
    ...
    const pictogramLabel =
      form.pictogramLabel.trim() || autoPictogramLabel(title);
    const payload = {
      ...form,
      title,
      pictogramLabel,
  ...
  return (
    <section
      aria-labelledby="calendar-routine-title"
      className="mt-4 border-t border-[#eee5f7] pt-3.5"
    >
      <h3
        id="calendar-routine-title"
        className="text-sm font-extrabold uppercase tracking-wide text-[#5f477c]"
      ...
      {!active ? (
        <div className="mt-2 rounded-2xl border border-dashed border-[#e0d8f0] bg-[#faf8ff] px-3 py-3">
          <p className="text-sm font-semibold text-[#4a4a5a]">
            No hay rutina asignada a los {dayNames[dayOfWeek]}.
          </p>
          <button
            type="button"
            onClick={() => {
              const id = addRoutine(`Rutina ${dayNames[dayOfWeek]}`, dayOfWeek);
              setActiveId(id);
            }}
          ...
          {routines.length > 0 && (
            <div className="mt-3">
              <p className="text-[11px] text-[#8b7aa0] mb-1">
              ...
              >
                ...
                {routines
                  .filter((r) => r.id !== bondedId)
                  .map((r) => (
                    <option key={r.id} value={r.id}>
                      {r.name} (
                      {r.dayOfWeek !== null ? dayNames[r.dayOfWeek] : "Sin día"}
                      )
                    </option>
                  ))}
              </select>
        ...
        <div className="mt-2 space-y-3">
          <div className="bg-white rounded-2xl border border-[#f0e8f8] p-3">
            {!editingDay ? (
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    ...
                    className="p-2 rounded-xl hover:bg-[#f5f0ff] text-[#8b7aa0]"
                    title="Editar rutina"
                  >
                    <Pencil size={14} />
                  </button>
                  <button
                    type="button"
                    onClick={() => duplicateRoutine(active.id)}
                    className="p-2 rounded-xl hover:bg-[#f5f0ff] text-[#8b7aa0]"
                    title="Duplicar"
                  >
                    <Copy size={14} />
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (confirm("¿Eliminar esta rutina?")) {
                        deleteRoutine(active.id);
                        setActiveId(bondedId);
                      }
                    }}
              ...
              <div className="space-y-2">
                <input
                  ...
                  placeholder="Nombre"
                  className="w-full p-2 rounded-xl border border-[#ede4f8] bg-[#faf8ff] text-sm text-[#4a4a5a] outline-none focus:border-[#6b4c9a]/30"
                />
                <select
                  value={dayDow}
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={saveDayEdit}
                    className="flex-1 py-2 rounded-xl bg-[#6b4c9a] text-white text-xs font-semibold hover:bg-[#5a3c8a] inline-flex items-center justify-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded-xl border border-[#ede4f8] text-xs text-[#6b4c9a] font-semibold bg-[#faf8ff] hover:bg-[#f5f0ff]"
                  >
                    Cancelar
                  </button>
                </div>
            ...
            <div className="mt-3 flex justify-between items-center mb-1.5">
              <span className="text-xs font-medium text-[#4a4a5a]">
                Progreso
              </span>
              <span className="text-xs font-bold text-[#6b4c9a]">
            ...
            <div className="w-full bg-[#f0e8f8] rounded-full h-2.5">
              <motion.div
                className="bg-[#6b4c9a] h-2.5 rounded-full"
                initial={{ width: 0 }}
          ...
          {showAddItem && (
            <motion.div
              ...
              className="bg-white rounded-2xl p-3 border border-[#6b4c9a]/20 shadow-lg space-y-2.5"
            >
              <div className="flex items-center justify-between">
                <h4 className="font-semibold text-sm text-[#4a4a5a]">
                  {editingItem ? "Editar paso" : "Nuevo paso"}
                </h4>
                <button type="button" onClick={() => setShowAddItem(false)}>
                  <X size={16} className="text-[#8b7aa0]" />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="p-2 rounded-xl border border-[#ede4f8] bg-[#faf8ff] text-sm text-[#4a4a5a] outline-none focus:border-[#6b4c9a]/30"
                />
                <SectionSelector
                  value={form.category}
                ...
                <div className="flex gap-1.5 overflow-x-auto pb-0.5">
                  {liveSuggestions.map((picto) => (
                    ...
                    >
                      ...
                      <span className="text-[9px] text-[#8b7aa0] max-w-[3.5rem] truncate">
                        {picto.name}
                      </span>
                    </button>
                  ))}
              ...
              <input
                value={form.pictogramLabel}
                onChange={(e) =>
                  setForm((f) => ({ ...f, pictogramLabel: e.target.value }))
                }
                placeholder="Etiqueta para pictograma (opcional)"
                className="w-full p-2 rounded-xl border border-[#ede4f8] bg-[#faf8ff] text-sm text-[#4a4a5a] outline-none focus:border-[#6b4c9a]/30"
              />
              <div>
                <p className="text-xs text-[#8b7aa0] mb-1 flex items-center gap-2">
                  ...
                  {manualPictogram && (
                    <span className="inline-flex items-center gap-1 text-[#6b4c9a] font-medium">
                      ...
                      <button
                        type="button"
                        onClick={() => setManualPictogram(null)}
                        className="text-[#8b7aa0] hover:text-red-500"
                      >
                        ✕
                      </button>
                    </span>
                        ...
                        onClick={() =>
                          setForm((current) => ({
                            ...
                            reminders: selected
                              ...
                              : [...current.reminders, choice.value].sort(
                                  (a, b) => a - b,
                                ),
                          }))
                        }
                        className={`rounded-lg border px-2 py-1 text-[11px] font-medium ${selected ? "border-[#6b4c9a] bg-[#f5f0ff] text-[#6b4c9a]" : "border-[#ede4f8] text-[#8b7aa0]"}`}
                      >
                        {choice.label}
                      </button>
                    );
              ...
              <button
                type="button"
                onClick={submitItem}
                className="w-full py-2 rounded-xl bg-[#6b4c9a] text-white text-sm font-semibold shadow-md shadow-purple-200 hover:bg-[#5a3c8a]"
              >
          ...
          {!showAddItem && (
            <button
              type="button"
              onClick={openCreate}
              className="w-full py-2.5 rounded-xl border-2 border-dashed border-[#e0d8f0] text-[#8b7aa0] hover:border-[#6b4c9a] hover:text-[#6b4c9a] hover:bg-[#faf8ff] text-sm inline-flex items-center justify-center gap-2"
            >
          ...
          {active.items.length > 0 && (
            <div className="space-y-3">
              {allCategories.map((cat) => {
                const catItems = active.items.filter(
                  (it) => it.category === cat,
                );
                if (catItems.length === 0) return null;
                return (
                  <div key={cat} className="space-y-1.5">
                    <h4 className="text-xs font-bold text-[#6b4c9a] uppercase tracking-wide">
                      {allLabels[cat] || cat}
                    </h4>
                    {!pictogramView ? (
                          ...
                          >
                            ...
                            >
                              {item.completed ? (
                                ...
                                <Circle size={19} className="text-[#8b7aa0]" />
                              )}
                            </button>
                            ...
                            <div className="flex-1 text-left min-w-0">
                              <p
                                className={`text-[13px] font-medium truncate ${item.completed ? "line-through text-[#8b7aa0]" : "text-[#4a4a5a]"}`}
                              >
                                {item.title}
                              </p>
                              {(item.reminders?.length || 0) > 0 && (
                                <p className="text-[10px] text-[#6b4c9a]">
                                  🔔 {item.reminders!.length} aviso
                                  {item.reminders!.length === 1 ? "" : "s"}
                                </p>
                              )}
                            </div>
                            <span className="text-[11px] text-[#8b7aa0] flex items-center gap-0.5">
                              <Clock size={11} /> {item.time}
                            </span>
                            <div className="flex gap-0.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                              <SpeakButton
                                text={item.pictogramLabel || item.title}
                                size={11}
                                className="p-1.5"
                              />
                              <button
                                ...
                                className="p-1.5 rounded-lg hover:bg-[#f5f0ff] text-[#8b7aa0]"
                                title="Editar"
                              >
                                <Pencil size={11} />
                              </button>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {catItems.map((item, i) => (
                          ...
                          >
                            ...
                            <span className="text-xs font-semibold text-[#4a4a5a] text-center leading-tight">
                              {item.pictogramLabel || item.title}
                            </span>
                            <span className="text-[10px] text-[#8b7aa0] flex items-center gap-0.5">
                            ...
                            <span className="absolute top-1 right-1">
                              {item.completed ? (
                                ...
                                <Circle size={15} className="text-[#8b7aa0]" />
                              )}
                            </span>
                            <div className="absolute top-1 left-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button
                                ...
                                className="p-0.5 rounded hover:bg-[#f5f0ff] text-[#8b7aa0]"
                              >
                                <Pencil size={9} />
                              </button>
                              <button
                            ...
                            {!item.completed && (
                              ...
                              >
                                <StartTaskHint
                                  stepTitle={item.pictogramLabel || item.title}
                                />
                              </div>
                            )}
                          </motion.div>
